...
export class Forest extends Ecs.World {
    ...
    * iter(entity: Entity, kind: Node.NodeKind | null = null): IterableIterator<Node.Node> {
    ...
    private resize_fork_(ext: Ext, child_e: Entity, crect: Rectangle, shrunk?: movement.Movement) {
        ...
        if (shrunk) {
            const origin = shrunk === Movement.DOWN
                ? child.area.y
                : shrunk === Movement.UP
                    ? child.area.y + child.area.height
                    : shrunk === Movement.LEFT
                        ? child.area.x
                        : child.area.x + child.area.width;

            while (parent !== null) {
                child = this.forks.get(parent) as Fork.Fork;
                is_left = child.left.is_fork(child_e);

                if (shrunk === Movement.DOWN) {
                    if (origin > child.area.y) break
                } else if (shrunk === Movement.UP) {
                    if (origin < child.area.y + child.area.height) break
                } else if (shrunk === Movement.LEFT) {
                    if (origin > child.area.x) break
                } else if (origin < child.area.x + child.area.width) {
                    break
                }
